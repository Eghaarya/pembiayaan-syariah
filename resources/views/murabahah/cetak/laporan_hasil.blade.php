...
<body contenteditable="true" spellcheck="false" class="bg-white text-dark">
    ...
    <div class="container p-5">
        ...
        <table class="table table-bordered table-sm w-100 text-center text-center" style="table-layout: fixed;">
            <thead class="small">
                <tr>
                    <td colspan="3" class="text-left p-1" style="color: red">Penghasilan</td>
                </tr>
                <tr>
                    <th style="text-transform: none;" class="bg-white align-middle text-wrap p-1 m-0">
                        Penghasilan</th>
            ...
            <tbody class="small" id="penghasilan-section">
                @foreach ($penghasilan_fields as $field)
                    <tr>
                        ...
                        <td class="p-1">
                            <input type="text" maxlength="50" name="keterangan_{{ $field['name'] }}"
                                class="form-control border-0 bg-white text-dark form-control-sm"
                                value="{{ old('keterangan_' . $field['name'], $pengajuan_capacity->{'keterangan_' . $field['name']} ?? '') }}">
                        </td>
                    </tr>
                @endforeach
            ...
            <thead class="small">
                <tr class="bg-light font-weight-bold">
                    <td colspan="3" class="text-left p-1" style="color: red">Pengeluaran</td>
                </tr>
                <tr>
            ...
            <tbody class="small" id="pengeluaran-section">
                @foreach ($pengeluaran_fields as $field)
                    <tr>
                        ...
                        <td class="p-1">
                            <input type="text" maxlength="50" name="keterangan_{{ $field['name'] }}"
                                class="form-control border-0 bg-white text-dark align-middle form-control-sm"
                                value="{{ old('keterangan_' . $field['name'], $pengajuan_capacity->{'keterangan_' . $field['name']} ?? '') }}">
                        </td>
                    </tr>
                @endforeach
        ...
        <h6 class="border-bottom pb-2 mt-3">Maksimal Angsuran
        </h6>
        <table class="table">
            <tbody>
                <tr>
                    ...
                    <td class="align-middle p-1" id="maks-angsuran-1">Rp.
                        0,00
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="align-middle p-1" id="maks-angsuran-2">Rp.
                        0,00
                    </td>
                </tr>
            </tbody>
        ...
        <h6 class="border-bottom pb-2 mt-3">Kemauan Membayar</h6>
        @php
            $capacityKemauanMembayar = [
                'Tempat Kerja Thd Lokasi Bank' => $pengajuan_capacity->tempatkerja_kelokasi_bank ?? '-',
                'Tempat Kerja Thd Lokasi Agunan' => $pengajuan_capacity->tempatkerja_kelokasi_agunan ?? '-',
                'Pembayaran Kolektif' => $pengajuan_capacity->pembayaran_kolektif ?? '-',
                'Pembayaran Non Kolektif' => $pengajuan_capacity->pembayaran_nonkolektif ?? '-',
            ];
        @endphp

        @foreach ($capacityKemauanMembayar as $label => $value)
            <div class="row mb-1">
                <div class="col-4 fw-bold text-dark d-flex justify-content-between">
                    <span>{{ $label }}</span>
                    <span>:</span>
        ...
        @php
            ...
            $capitalPermohonanPembiayaan = [
                ...
                'Harga Jual Barang' => $pengajuan->permohonan_harga_jual_barang ?? '--',
                'Urbun / Uang Muka' => $pengajuan->permohonan_urbun_uangmuka ?? '--',
                ...
                'Besarnya Urbun' => $pengajuan_capital->besarnya_urbun ?? '--',
            ];
        @endphp
        ...
        <h6 class="border-bottom pb-2 mt-3">Pembiayaan Berdasarkan Gross
            Income</h6>

        <div class="row g-3 mb-3">
            <div class="col-6">Harga beli Bank/ Pembiayaan</div>
            <div class="col-6 text-left" id="grossincome_harga_beli_bank">
                Rp. 0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Margin Bank</div>
            <div class="col-6 text-left" id="grossincome_margin_bank">
                Rp. 0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Harga Jual Bank</div>
            <div class="col-6 text-left" id="grossincome_harga_jual_bank">Rp.
                0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">
                <label for="grossincome_jangka_waktu_pembiayaan">Jangka Waktu
                    Pembiayaan
                    (Tahun)</label>
            </div>
            <div class="col-6">
                <input disabled type="text" class="form-control border-0 bg-white text-left p-0 m-0"
                    id="grossincome_jangka_waktu_pembiayaan" name="grossincome_jangka_waktu_pembiayaan"
        ...
        <div class="row g-3 mb-3">
            <div class="col-6">Jangka Waktu Pembiayaan
                (Bulan)</div>
            <div class="col-6 text-left" id="grossincome_jangka_waktu_bulan">12
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Angsuran Per Bulan</div>
            <div class="col-6 text-left" id="grossincome_angsuran_per_bulan">Rp.
                0,00</div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Repayment Capacity (kemampuan gaji
                nasabah membayar
                angsuran)</div>
            <div class="col-6 text-left" id="grossincome_repayment">1</div>
        </div>
        ...
        <div class="row g-3 mb-3">
            <div class="col-6">Harga Jual Bank</div>
            <div class="col-6 text-left" id="netincome_harga_jual_bank">Rp.
                0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
        ...
        <h6 class="border-bottom pb-2 mt-3">Pembiayaan Berdasarkan
            Income(terkecil dari Net
            atau
            Gross)
        </h6>
        ...
        <div class="row g-3 mb-3">
            <div class="col-6">Harga beli Bank/ Pembiayaan</div>
            <div class="col-6 text-left" id="terkecil_harga_beli_bank">
                Rp. 0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Margin Bank</div>
            <div class="col-6 text-left" id="terkecil_margin_bank">
                Rp. 0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Harga Jual Bank</div>
            <div class="col-6 text-left" id="terkecil_harga_jual_bank">Rp.
                0,00
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">
                <label for="terkecil_jangka_waktu_pembiayaan">Jangka Waktu Pembiayaan
                    (Tahun)</label>
            </div>
            <div class="col-6">
                <input disabled type="text" class="form-control border-0 bg-white text-left p-0 m-0"
                    id="terkecil_jangka_waktu_pembiayaan" name="terkecil_jangka_waktu_pembiayaan"
                    value="{{ ': ' . $pengajuan->permohonan_jangka_waktu_pembiayaan . ' tahun' }}"
                    oninput="hitungHargaJual()">
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Jangka Waktu Pembiayaan
                (Bulan)</div>
            <div class="col-6 text-left" id="terkecil_jangka_waktu_bulan">12
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Angsuran Per Bulan<span class="font-italic">(diambil
                    angsuran terkecil
                    pada maksimal angsuran)</span></div>
            <div class="col-6 text-left" id="terkecil_angsuran_per_bulan">Rp.
                0,00</div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">Repayment Capacity (kemampuan gaji
                nasabah membayar
                angsuran)</div>
            <div class="col-6 text-left" id="terkecil_repayment">1
            </div>
        </div>
        ...
        <div class="row g-3 mb-3">
            <div class="col-6">Harga Jual Bank</div>
            <div class="col-6">
                <input type="hidden" id="analis_harga_jual_bank_from_db"
                    value="{{ $pengajuan_capacity->analis_harga_jual_bank ? '1' : '0' }}">

                <input type="number" step="0.01"
                    class="form-control border-0 bg-white text-dark text-left p-0 pl-2 py-1 m-0"
                    id="analis_harga_jual_bank" name="analis_harga_jual_bank"
                    value="{{ old('analis_harga_jual_bank', $pengajuan_capacity->analis_harga_jual_bank ?? '') }}">
            </div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-6">
        ...
        {{-- Start Collateral KPR --}}
        <h5 class="mb-0 py-1 mt-3">COLLATERAL KPR</h5>
        <h6 class="border-bottom pb-2">Legalitas Obyek/ Agunan Pembiayaan</h6>
        @php
            $collateralkprLegalitasObyek = [
                'Jenis Sertifikat Hak' => $pengajuan_collateralkpr->jenis_sertifikat_hak ?? '-',
                'Nomor Sertifikat' => $pengajuan_collateralkpr->nomor_sertifikat ?? '-',
                'Tanggal Penerbitan' => $pengajuan_collateralkpr->tanggal_penerbitan
                    ? \Carbon\Carbon::parse($pengajuan_collateralkpr->tanggal_penerbitan)->format('d/m/Y')
                    : '-',
                'Instansi yang Menerbitkan' => $pengajuan_collateralkpr->instansi_yang_menerbitkan ?? '-',
                'Nama Pemegang Hak' => $pengajuan_collateralkpr->nama_pemegang_hak ?? '-',
                'Lama & Tgl Akhir Hak Berlaku' => $pengajuan_collateralkpr->lama_tgl_akhir_hak_berlaku ?? '-',
                'Surat Ukur Nomor' => $pengajuan_collateralkpr->surat_ukur_nomor ?? '-',
                'Tanggal Ukur' => $pengajuan_collateralkpr->tanggal_ukur
                    ? \Carbon\Carbon::parse($pengajuan_collateralkpr->tanggal_ukur)->format('d/m/Y')
                    : '-',
                'Asal Agunan' => $pengajuan_collateralkpr->asal_agunan ?? '-',
                'Luas Agunan' => $pengajuan_collateralkpr->luas_agunan ?? '-',
                'Letak Agunan' => $pengajuan_collateralkpr->letak_agunan ?? '-',
                'Batas Utara Agunan' => $pengajuan_collateralkpr->batas_utara_agunan ?? '-',
                'Batas Timur Agunan' => $pengajuan_collateralkpr->batas_timur_agunan ?? '-',
                'Batas Selatan Agunan' => $pengajuan_collateralkpr->batas_selatan_agunan ?? '-',
                'Batas Barat Agunan' => $pengajuan_collateralkpr->batas_barat_agunan ?? '-',
            ];
        @endphp

        @foreach ($collateralkprLegalitasObyek as $label => $value)
            <div class="row mb-1">
                <div class="col-4 fw-bold text-dark d-flex justify-content-between">
                    <span>{{ $label }}</span>
                    <span>:</span>
        ...
        @php
            $collateralkprKondisiAgunan = [
                'Aksesibilitas Lokasi Agunan' => ucfirst($pengajuan_collateralkpr->aksesibilitas_lokasi_agunan ?? '-'),
                'Keadaan Lingkungan Agunan (Tanah)' => ucfirst(
                    $pengajuan_collateralkpr->keterangan_lingkungan_agunan_tanah ?? '-',
                ),
                'Keadaan Lingkungan Agunan (Kawasan)' => ucfirst(
                    $pengajuan_collateralkpr->keterangan_lingkungan_agunan_kawasan ?? '-',
                ),
                'Penggunaan Agunan Saat Ini' => ucfirst($pengajuan_collateralkpr->penggunaan_agunan_saat_ini ?? '-'),
                'Harga Sewa per Tahun' => $pengajuan_collateralkpr->harga_sewa_per_tahun
                    ? 'Rp. ' . number_format($pengajuan_collateralkpr->harga_sewa_per_tahun, 0, ',', '.')
                    : '-',
                'Akses Jalan ke Jalan Besar' => ucfirst(
                    $pengajuan_collateralkpr->agunan_punya_akses_jalan_besar ?? '-',
                ),
                'Termasuk Aktiva Warisan Belum Dibagi' =>
                    $pengajuan_collateralkpr->agunan_aktiva_warisan_belum_dibagi ?? '-',
            ];
        @endphp

        @foreach ($collateralkprKondisiAgunan as $label => $value)
            <div class="row mb-1">
                <div class="col-4 fw-bold text-dark d-flex justify-content-between">
                    <span>{{ $label }}</span>
                    <span>:</span>
        ...
        @php
            $collateralkprBangunanAgunan = [
                'IMB (Izin Mendirikan Bangunan)' => $pengajuan_collateralkpr->memiliki_imb ?? '-',
                'Tahun Pembuatan Bangunan' => $pengajuan_collateralkpr->tahun_pembuatan_bangunan ?? '-',
                'Perkiraan Biaya Pembangunan (Rp)' => $pengajuan_collateralkpr->perkiraan_biaya_pembangunan
                    ? 'Rp. ' . number_format($pengajuan_collateralkpr->perkiraan_biaya_pembangunan, 0, ',', '.')
                    : '-',
                'Keterangan Konstruksi Bangunan' => $pengajuan_collateralkpr->keterangan_konstruksi_bangunan ?? '-',
                'Luas Efektif (m²)' => $pengajuan_collateralkpr->luas_efektif ?? '-',
                'Jumlah Lantai' => $pengajuan_collateralkpr->jumlah_lantai ?? '-',
                'Pondasi' => $pengajuan_collateralkpr->pondasi ?? '-',
                'Lantai' => $pengajuan_collateralkpr->lantai ?? '-',
                'Konstruksi' => $pengajuan_collateralkpr->konstruksi ?? '-',
                'Dinding' => $pengajuan_collateralkpr->dinding ?? '-',
                'Dinding Pemisah' => $pengajuan_collateralkpr->dinding_pemisah ?? '-',
                'Kusen' => $pengajuan_collateralkpr->kusen ?? '-',
                'Pintu' => $pengajuan_collateralkpr->pintu ?? '-',
                'Jendela/Ventilasi' => $pengajuan_collateralkpr->jendela_ventilasi ?? '-',
                'Plafond' => $pengajuan_collateralkpr->plafond ?? '-',
                'Konstruksi Atap' => $pengajuan_collateralkpr->konstruksi_atap ?? '-',
                'Penutup Atap' => $pengajuan_collateralkpr->penutup_atap ?? '-',
                'Instalasi Air' => $pengajuan_collateralkpr->instalasi_air ?? '-',
                'Instalasi Listrik' => $pengajuan_collateralkpr->instalasi_listrik ?? '-',
                'Perawatan' => $pengajuan_collateralkpr->perawatan ?? '-',
                'Kondisi Sarana dan Emplasemen' => $pengajuan_collateralkpr->kondisi_sarana_dan_emplasemen ?? '-',
                'Informasi Lain tentang Kondisi Bangunan' =>
                    $pengajuan_collateralkpr->informasi_lain_kondisi_bangunan ?? '-',
            ];
        @endphp

        @foreach ($collateralkprBangunanAgunan as $label => $value)
            <div class="row mb-1">
                <div class="col-4 fw-bold text-dark d-flex justify-content-between">
                    <span>{{ $label }}</span>
                    <span>:</span>
        ...
        @php
            $collateralkprMarketabilitasAgunan = [
                'Lokasi Perumahan' => $pengajuan_collateralkpr->lokasi_perumahan ?? '-',
                'Kenyamanan' => $pengajuan_collateralkpr->kenyamanan ?? '-',
                'Lokasi Agunan' => $pengajuan_collateralkpr->lokasi_agunan ?? '-',
                'Jarak Fasum/Fasos' => $pengajuan_collateralkpr->jarak_fasum_fasos ?? '-',
                'Fasilitas Perumahan' => $pengajuan_collateralkpr->fasilitas_perumahan ?? '-',
                'Jenis Jalan Lingkungan' => $pengajuan_collateralkpr->jenis_jalan_lingkungan ?? '-',
                'Jarak ke Jalan Provinsi' => $pengajuan_collateralkpr->jarak_ke_jalan_provinsi ?? '-',
                'Jenis dan Kondisi Jalan' => $pengajuan_collateralkpr->jenis_dan_kondisi_jalan ?? '-',
                'Kondisi Jalan ke Kota' => $pengajuan_collateralkpr->kondisi_jalan_ke_kota ?? '-',
                'Resiko Bencana Hidup' => $pengajuan_collateralkpr->resiko_bencana_hidup ?? '-',
                'Kontribusi Pemohon DP' => $pengajuan_collateralkpr->kontribusi_pemohon_dp ?? '-',
                'Pertumbuhan Agunan' => $pengajuan_collateralkpr->pertumbuhan_agunan ?? '-',
                'Kondisi Wilayah Agunan' => $pengajuan_collateralkpr->kondisi_wilayah_agunan ?? '-',
            ];
        @endphp

        @foreach ($collateralkprMarketabilitasAgunan as $label => $value)
            <div class="row mb-1">
                <div class="col-4 fw-bold text-dark d-flex justify-content-between">
                    <span>{{ $label }}</span>
                    <span>:</span>
            ...
            @foreach ($dokumentasiIdentitas as $label => $filename)
                @if ($filename)
                    <div class="col-6 mb-3 text-end">
                        ...
                        <img src="{{ asset('storage/uploads/murabahah/' . $filename) }}" alt="{{ $label }}"
        ...
        @php
            ...
            $keputusanPembiayaan = [
                ...
                'Harga Jual Barang' => $pengajuan->keputusan_harga_jual_barang ?? '--',
                'Urbun / Uang Muka' => $pengajuan->keputusan_urbun_uangmuka ?? '--',
        ...
        @php
            $hasilScooring = [
                ...
                'Total Collateral KPR' => $pengajuan->total_collateralkpr ?? 0,
